...
const Editcommondata = ({ setCommonDataEdit, commonData, fetchData }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const validateAndSubmit = async () => {
            try {
                const UpdateApi = await commonDataPatchApiWithData(commonData.id, updateCommonData);
        ...
        <div className={`d-flex justify-content-center align-items-center bg-opacity-75 h-100  w-100  position-fixed start-0 top-0 z-2 bg-white `} >
            <div className={`px-4 py-3 shadow rounded w-300 border-color bg-white`}>
                <div className='d-flex  justify-content-end'>
                    <button className='btn btn-link  m-0 py-0 px-0 ' onClick={() => setCommonDataEdit()}><img src={closeIcon} alt="close" /></button>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-floating mb-1">
                        <input type="text"
                            ...
                            id="key" placeholder="Key" name='key' onChange={handleChange} value={commonData.key} readOnly />
                        <label htmlFor="key">Key</label>
                    </div>
                    <div className="form-floating mb-1">
                        <input type="text"
                            className={`form-control focus-ring focus-ring-light text-color ${error === 'data' && 'border-danger'}`}
                            id="data" placeholder="Data" name='data' onChange={handleChange} required defaultValue={commonData.data} />
