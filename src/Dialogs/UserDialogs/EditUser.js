...
const Edituser = ({ setUserEditData, action, fetchData }) => {
    ...
    const validateAndSubmit = async () => {
        try {
            const updateApi = await userPutApiWithIdDate(action.id, updatedUser);
            if (updateApi.data) {
        ...
        <div className={`d-flex justify-content-center align-items-center bg-opacity-75 h-100  w-100  position-fixed start-0 top-0 z-2 bg-white`} >
            <div className={`px-4 py-3    shadow rounded w-300  bg-white color border-color `}>
                <div className='d-flex  justify-content-end'>
                    <button className='btn btn-link text-dark m-0 py-0 px-0 ' onClick={() => setUserEditData()}><img src={closeIcon} alt="close" /></button>
                </div>
                <h3 className='text-center  mb-4'>Update User</h3>
                <form onSubmit={handleSubmit}>
                    <div>
                        <div className="form-floating mb-3 ">
                            <input type="text" className={`form-control focus-ring focus-ring-light color ${error === 'name' && 'border-danger'} `} id="full_name" placeholder="Full Name" name='full_name' onChange={handleuserInput} defaultValue={action.full_name} required />
                            <label htmlFor="full_name">Full Name</label>
                        </div>
                        <div className="form-floating  mb-3">
                            <input type="number" className={`form-control focus-ring focus-ring-light  color ${error === 'number' && 'border-danger'}`} id="phone_number" placeholder="Phone Number" name='phone_number' defaultValue={action.phone_number} onChange={handleuserInput} maxLength={10}
                                onKeyDown={(e) => {
